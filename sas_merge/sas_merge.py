...
def SAS_merge(df1,df2,join_type,join_key):
    ...
    tablea = tablea.withColumn(
        ...
        #don't really need this offset.but good for troubleshooting
    ...
    df_left_exclusion_list = df_left_exclude.select(join_key).rdd.flatMap(lambda x: x).collect()
    df_right_exclusion_list = df_right_exclude.select(join_key).rdd.flatMap(lambda x: x).collect()
    ...
    tablea_groupby = tablea.groupby(join_key).count().withColumnRenamed("count","tablea_count")
    tableb_groupby = tableb.groupby(join_key).count().withColumnRenamed("count","tableb_count")
    #take out the unique keys
    tablea_groupby = tablea_groupby.filter(~col(join_key).isin(df_exclustion))
    tableb_groupby = tableb_groupby.filter(~col(join_key).isin(df_exclustion))
    #find the max count
    ...
    common_columns_wokey.remove(join_key)
    ...
    #filter when the max count is from either TableA or TableB
    ...
    #table_combined is the combined result with commmon key 
    ...
    #create contract account based row number for the unique identifier
    ...
    uncommon_tablea_columns_only.remove(join_key)
    uncommon_tableb_columns_only = uncommon_tableb_columns.copy()
    uncommon_tableb_columns_only.remove(join_key)
